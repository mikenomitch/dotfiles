...
| mise | runtimes (node/python) + global CLIs (pnpm, wrangler, claude-code) | `mise/.config/mise/config.toml` |
...
| Claude Code | global agent config | `claude/.claude/{settings.json,CLAUDE.md}` |
...
| macOS | system preferences (one-shot script, not stowed) | `macos.sh` (`make macos`) |
...
cd ~/dotfiles
make bootstrap     # Homebrew + Brewfile, symlinks configs, mise install, macOS defaults
make shell         # one-time: set fish as default shell (asks for sudo)
make touchid       # one-time: enable Touch ID for sudo (asks for sudo)
make vscode        # install VS Code extensions + the local Tomorrow Night Bright theme
make mcp           # optional: register Claude Code MCP servers
```

Then open Zed once — it auto-installs the extensions declared in `settings.json`.

`make bootstrap` runs a **preflight** first: if the Brewfile has App Store (`mas`)
apps, it checks you're signed into the App Store and bails early with instructions
if not — so bootstrap doesn't half-fail on the Bear line. (Manual check: `mas account`.)

> **macOS defaults** (`make macos`, also part of `bootstrap`): runs `macos.sh`, which
...
> the repo**. `make link` first ensures `~/.ssh` is a real directory (perms 700) so
> stow links just the `config` file into it rather than symlinking the whole `~/.ssh`.
> Machine-specific or sensitive entries go in `~/.ssh/config.local` (git-ignored,
> `Include`d first so it wins). Touch ID for sudo is `make touchid`.
...
> If `make link` complains about conflicts, you have real files where symlinks
...
- **A new config file** → drop it in the right stow package, then `make relink`.
...
```sh
cd ~/dotfiles && make update   # git pull, brew bundle, mise upgrade, relink
```
